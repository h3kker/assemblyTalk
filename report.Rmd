...
```{r abyssContigs, out.width=1000, fig.width=14, dependson=c('loadAbyss')}
contigStats(N=all.width, reflength=all.len, style='ggplot2')
```
...
```{r ceruleanContigs, out.width=1000, fig.width=14, dependson=c('loadCerulean')}
contigStats(N=all.width, reflength=all.len, style='ggplot2')
```
...
```{r sgaContigs, out.width=1000, fig.width=14, dependson=c('loadSGA')}
tmp.subset<-c('contigs', 'scaffolds', 'sga.contigs', 'sga.scaffolds')
contigStats(N=all.width[tmp.subset], reflength=all.len[tmp.subset], style='ggplot2')
```
...
```{r soapContigs, out.width=1000, fig.width=14, dependson=c('loadSOAP')}
...
```{r pbjContigs, out.width=1000, fig.width=14, dependson=c('loadPBJelly')}
...
```{r postPbjGapStatsPlot, dependson=c('postPbjGapStats'), dpi=150}
ggplot(all.gaps, aes(x=length, y=gapwidth/length))+geom_point(aes(size=gaps, color=set))
```
